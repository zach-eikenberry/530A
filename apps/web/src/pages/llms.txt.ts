import { CANONICAL_LINKS, CANONICAL_ORIGIN, contentCorpus, RULES_VERIFIED_AT } from '@530a/config'
...
export const GET: APIRoute = () => {
  // Answer pages come from the shared corpus (one title/URL per canonical
  // doc), so this file can't drift from what the MCP server serves.
  const answerPages = [
    ...new Map(
      contentCorpus(CANONICAL_ORIGIN)
        .filter((d) => !d.id.startsWith('faq-'))
        .map((d) => [d.url, `- [${d.title}](${d.url})`]),
    ).values(),
  ].join('\n')
...
- [Why free](${CANONICAL_ORIGIN}/why-free): no ads, no personal data collection, public-good project
- [Terms](${CANONICAL_ORIGIN}/terms) · [Privacy](${CANONICAL_ORIGIN}/privacy)

## Canonical answer pages (from the shared content corpus)
